...
- `docker-compose.ci-cache.yml` is an additive Compose override, opted into only via the `COMPOSE_FILE` env var on specific CI steps, adding GHA buildx `cache_from`/`cache_to` for services that still build locally on the fallback path — including inside `test-ai4realnet-orchestrator-railway`'s externally-vendored fixture, which calls `DockerCompose()` with no explicit `-f` and so picks up `COMPOSE_FILE` transparently. Never applied to local dev: the `type=gha` cache backend needs `ACTIONS_CACHE_URL`/`ACTIONS_RUNTIME_TOKEN`, only present inside GH Actions runners. Neither this nor `docker-compose.ci-prebuilt.yml` is baked directly into the base `docker-compose.yml` — that would change default behavior for every consumer (the README's self-hosting quickstart, local dev, the other jobs' fallback path), not just the CI jobs that opt in. Only `fab-backend`/`fab-frontend`'s cache scopes are actually shared with another workflow (`build-images`, `fab-docker.yml`, `release-please.yml` all write the same `scope=fab-backend`/`scope=fab-frontend`) — `flatland3-benchmarks-orchestrator` and `ai4realnet-orchestrator-railway` each get their own dedicated, unshared scope here (see that file's header comment for the full breakdown of why).
