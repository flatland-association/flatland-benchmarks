...
export class Schema {
  ...
  static async setup(this: typeof Schema) {
    ...
    await this.migrate('EXTENSION uuid-ossp')`
      CREATE EXTENSION IF NOT EXISTS "uuid-ossp"
    `

    await this.migrate('TABLE SCHEMA submissions v3')`
      ALTER TABLE submissions
    ...
    await this.migrate('TABLE SCHEMA submissions v4')`
      ALTER TABLE submissions
    ...
    await this.migrate('TABLE SCHEMA results v2')`
      ALTER TABLE results
        ADD COLUMN IF NOT EXISTS uuid uuid DEFAULT uuid_generate_v4(),
    ...
    await this.migrate('TABLE VALUES results v2')`
      UPDATE results
        SET submission_uuid = submissions.uuid
        FROM (
          SELECT submissions.uuid
            FROM results
            LEFT JOIN submissions ON results.submission = submissions.id
        ) AS submissions
    ...
    await this.migrate('TABLE INDEX results (submission) v2')`
