...
export class BenchmarkController extends Controller {
  ...
  getBenchmarks: GetHandler<'/definitions/benchmarks'> = async (req, res) => {
    ...
    const benchmarks = await sql.query<BenchmarkDefinitionRow>`
      SELECT benchmarks.id, benchmarks.name, benchmarks.description, benchmarks.contents, benchmarks.field_ids, benchmarks.campaign_field_ids, benchmarks.test_ids, benchmarks.deadline, suites.id as suite_id FROM benchmarks
